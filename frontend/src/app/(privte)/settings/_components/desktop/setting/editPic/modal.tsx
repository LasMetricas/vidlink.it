...
const Modal: React.FC<Type> = ({
  ...
  function handleUploadImg(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        errorModal("Invalid file type. Please upload an image.");
        return;
      }
      setImgFile(file);
  ...
  return (
    <>
      <ul
        ref={menuRef}
        className="absolute top-[0px] -left-[5px] flex flex-col justify-between bg-[#191919] text-[16px] tracking-normal font-semibold w-[260px] h-[122px] px-[15px] py-[10px] border-[1px] rounded-[9.1px]"
      >
        <li className="flex items-center gap-[10px]">
          <img className="size-[24px]" src="/icon/profile/library.png" alt="" />
        ...
        />
        <li className="flex items-center gap-[10px]">
          <img className="size-[24px]" src="/icon/profile/take.png" alt="" />
